...
echo "> 컨테이너 ID는 무엇?? ${CONTAINER_ID}" >> $DEPLOY_LOG
echo "> 현재 프로필은 무엇?? ${IDLE_PROFILE}" >> $DEPLOY_LOG

if [ -z ${CONTAINER_ID} ]
then
  echo "> 현재 구동중인 애플리케이션이 없으므로 종료하지 않습니다." >> $DEPLOY_LOG
else
  echo "> docker stop ${IDLE_PROFILE}" >> $DEPLOY_LOG
  sudo docker stop ${IDLE_PROFILE}
  echo "> docker rm ${IDLE_PROFILE}" >> $DEPLOY_LOG
